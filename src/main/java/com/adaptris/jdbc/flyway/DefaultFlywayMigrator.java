...
public class DefaultFlywayMigrator implements FlywayMigrator {
  ...
  public void migrate(DataSource source) throws Exception {
    FluentConfiguration configuration = Flyway.configure().dataSource(source).locations(getFlywayLocations().toArray(new String[] {}));
   ...
   *
   * <p>
   * Locations can be prefixed with classpath or filesytem. which means that {@code filesystem:./sql/} if the files are on the filesystem or
   * {@code classpath:migration/mysql} if they are on the classpath.
   * </p>
   *
   ...
   *
   * @param baseline
   *          true to baseline, default is false if not explicitly configured.
   */
   ...
   *
   * @param flywayTable
   *          the table. optional. If not provided will use the default flyway history table
   */
